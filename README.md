...
```bash
sam-cicd-exp$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-cicd-exp$ sam local start-api
sam-cicd-exp$ curl http://localhost:3000/
```
...
```bash
sam-cicd-exp$ sam logs -n HelloWorldFunction --stack-name sam-cicd-exp --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-cicd-exp
```
